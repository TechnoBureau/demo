...
get_release_id() {
  ...
  curl -sk -H "Authorization: Bearer $github_token" \
    "https://api.github.technobureau.com/repos/$GITHUB_REPOSITORY/releases/tags/$release_version" |
    jq -r '.id'
}
get_asset_id() {
  local release_id="$1"
  local asset_name="$2"
  local asset_id=$(curl -L -sk -H "Authorization: Bearer $token" "https://api.github.technobureau.com/repos/$GITHUB_REPOSITORY/releases/$release_id/assets" | jq -r '.[] | select(.name == '\"$asset_name\"') | .id')
  echo $asset_id
}
download_asset() {
  local release_id="$1"
  ...
  if [[ ! -z "$asset_id" ]]; then
    curl -sk -L \
      ...
      "https://api.github.technobureau.com/repos/$GITHUB_REPOSITORY/releases/assets/$asset_id" > "$asset_name"
    ...
    if [[ ! -z "$delete_flag" ]]; then
      ...
      curl -sk -L -X DELETE \
        ...
        "https://api.github.technobureau.com/repos/$GITHUB_REPOSITORY/releases/assets/$asset_id" > /dev/null
    fi
  fi
}
...
upload_asset() {
  ...
  curl -sk -L -X POST \
    ...
    "https://github.technobureau.com/api/uploads/repos/$GITHUB_REPOSITORY/releases/$release_id/assets?name=$asset_name" > /dev/null
...
update_release_notes() {
  ...
  curl -sk -X PATCH \
    ...
    -H "Accept: application/vnd.github.v3+json" \
    -d '{"body": "'"$body"'"}' \
    "https://api.github.technobureau.com/repos/$GITHUB_REPOSITORY/releases/$release_id" > /dev/null
}
